    ...
    <div className={`rounded-2xl border p-5 ${dark ? 'bg-corporate-surface-dark border-corporate-border-dark' : 'bg-white border-corporate-bg'}`}>
      ...
      {loading ? (
        ...
        <div className="max-h-64 overflow-y-auto space-y-1">
          {codes.map((c) => (
            <div key={c.code} className={`flex items-center justify-between px-3 py-2 rounded-lg ${dark ? 'hover:bg-corporate-nav-dark' : 'hover:bg-corporate-bg'}`}>
              <div className="flex items-center gap-2">
                {c.redeemed
                  ? <CheckCircle2 size={14} className="text-emerald-500" />
                  : <Circle size={14} className={dark ? 'text-white/20' : 'text-gray-300'} />}
                <span className={`font-mono text-xs ${dark ? 'text-white' : 'text-corporate-text-on-bg'}`}>{c.code}</span>
              </div>
